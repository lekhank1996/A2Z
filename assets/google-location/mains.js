			 ...
			 cache: false,
		 	 success: function(datas){
				    ...
				    $.each(obj, function( index, value ) {					   
 					$('<div class="form-group"><div class="radio"><label><input type="radio" name="taxi_type" value="'+value['cartype']+'"  class="second"><span><img src="'+value['car_image']+'"/></span><span>'+value['cartype']+'</span><span class="pr-car-nw">'+value['cartype']+value['cartype']+'</span><span class="fre-det-mn">'+value['total_rate']+'</span></label></div></div><input type="hidden" value='+value['timetype']+' name="timetype" id="timetype"><input type="hidden" value='+value['total_dist']+' name="distance" id="total_dis"><input type="hidden" value='+value['total_rate']+' name="amount" id="total_rate">').appendTo(".append_val");
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data==0){
				...
				$('#'+fields_message).html('Promo Code Valid');
				$('#'+fields).val(data);
			}		
